BacktestDMA <- function(object, iBurnPeriod = NULL){
  ...
  mOut = matrix(, 3, 2, dimnames = list(c("MSE", "MAD", "Predictive Likehood"), c("DMA", "DMS")))
  ...
  mOut["Predictive Likehood", "DMA"] = sum(vLpdf_DMA)
  mOut["Predictive Likehood", "DMS"] = sum(vLpdf_DMS)
